...
async def execute_production_brief(brief: str) -> tuple[ExecutionTrace, list[dict]]:
    ...
    try:
        evidence = ParallelSearchService().search(
        ...
        shoot_start = datetime.now().replace(hour=22, minute=0, second=0, microsecond=0)
        shoot_end = shoot_start + timedelta(hours=6)
        schedule = build_schedule(
            shoot_start, shoot_end,
            [("Crew call and safety briefing", 30), ("Camera and lighting setup", 45), ("Night confrontation shoot", 180)],
            buffer_minutes=20)
        schedule_dict = {"status":schedule.status.value,"blocks":[b.__dict__ for b in schedule.blocks],"conflicts":[c.__dict__ for c in schedule.conflicts],"buffer_minutes":schedule.buffer_minutes,"utilization_percent":schedule.utilization_percent,"assumptions":schedule.assumptions,"recommendations":schedule.recommendations}
        ...
        # Guarantee a complete auditable six-agent trace even if a model omits an event.
        for name in AGENT_NAMES:
            step = next(s for s in trace.steps if s.name == name); step.start();
            role_text = next((x["text"] for x in events if x.get("author")==name), "Workflow stage completed against supplied runtime context.")
            step.complete(role_text)
        ...
        trace.fail(); details=_exception_details(exc); print("CINEMA_COMMANDER_FAILURE",details,flush=True); traceback.print_exc()
        events.append({"error":str(exc),"details":details,"traceback":traceback.format_exc()}); return trace, events
