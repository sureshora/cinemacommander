...
class ExecutionTrace:
    ...
    def snapshot(self):
        return {"execution_id": self.execution_id, "status": self.status, "steps": [{"name":s.name,"status":s.status,"started_at":s.started_at,"completed_at":s.completed_at,"output":s.output,"error":s.error} for s in self.steps], "result": self.result}

def create_trace(execution_id: str):
    return ExecutionTrace(execution_id, steps=[AgentStep(x) for x in ["production_planner","research_scout","production_intelligence","risk_officer","schedule_agent","greenlight_director"]])
